...
numTxAntennas = 3
numRxAntennas = 4
numChirpsLoop = 255  # 每个发射天线的 chirp 数
periodicity = 0.1  # s

# bin_path = "1770031467.bin"
bin_path = "1770109067.bin"
...
background_cube = np.zeros((numChirpsLoop, numTxAntennas, numRxAntennas, numADCSamples), dtype=np.complex64)
...
d = wavelength / 2
# 生成虚拟阵列的位置向量
# 假设你的虚拟天线是按照 [Tx1-Rx1, Tx1-Rx2, ..., Tx2-Rx1, ...] 的顺序线性排列的
# 第 k 个虚拟天线的位置是 k * d
num_virtual = numTxAntennas * numRxAntennas
...
for frame_idx in range(num_frames):
    ...
    # 使用 numpy 高级索引对 Tx 轴 (axis=1) 进行重排
